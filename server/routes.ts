...
[중요] 참고 문서가 제공된 경우, 해당 문서의 인사말 스타일과 표현을 참고하되 똑같이 복사하지 마세요.

[작성 지침]
- 학부모님과 학생에게 전달하는 공문서 형식의 격식체
- 계절에 맞는 인사 (봄/여름/가을/겨울)
- 학교 교육에 대한 관심 감사
- 시험의 중요성과 성공적인 응시를 위한 안내임을 명시
- 학생들의 노력에 대한 격려
- 매번 다른 표현과 문장 구조로 작성
- 4~6문장, 줄바꿈 없이 작성
...
[중요] 참고 문서가 제공된 경우, 해당 학교의 유의사항 작성 스타일을 참고하여 다양하게 작성하세요.
...
[작성 지침]
- 참고 문서의 표현 방식을 활용하되 그대로 복사하지 마세요
- 학교별 특성에 맞게 표현을 다양하게 변형
- 8~12개 항목으로 구성
- 번호 또는 기호로 구분
- 매번 다른 순서와 표현 사용
...
유의사항 텍스트만 출력하세요 (제목이나 설명 없이).`;
          ...
          prompt = `당신은 고등학교 교무부 담당 교사입니다.
수능 또는 모의평가 시험 시간표를 생성해주세요.

[입력 정보]
${contextDescription || "시험 유형 정보가 아직 입력되지 않았습니다."}

[중요] 참고 문서가 제공된 경우, 해당 문서의 시간표 형식과 내용을 참고하여 작성하세요.

[작성 지침]
- 한국교육과정평가원 공식 시간표 기준으로 작성
- 1교시~5교시 및 점심시간 포함
- 각 교시별 시험 영역, 시간, 문항 수 명시
- 비고란에 특이사항 기재 (선택과목, 외출금지 등)

[출력 형식]
JSON 배열로 출력. 코드블록 없이 순수 JSON만:
[{"period":"교시","subject":"영역","time":"시간","questions":"문항수","note":"비고"}]`;
        } else if (fieldName === "supplies") {
          prompt = `당신은 고등학교 교무부 담당 교사입니다.
수능 또는 모의평가 응시에 필요한 준비물 목록을 생성해주세요.

[입력 정보]
${contextDescription || "시험 유형 정보가 아직 입력되지 않았습니다."}

[중요] 참고 문서가 제공된 경우, 해당 문서의 준비물 안내를 참고하여 학교 실정에 맞게 작성하세요.

[작성 지침]
- 필수 준비물 (신분증, 수험표, 필기구 등) 포함
- 학교별 추가 안내사항 반영 가능
- 5~8개 항목으로 구성
- 각 항목에 구체적인 설명 추가 (예: 어떤 종류의 신분증이 가능한지)

[출력 형식]
JSON 문자열 배열로 출력. 코드블록 없이 순수 JSON만:
["준비물1 (상세 설명)","준비물2","준비물3"]`;
        } else if (fieldName === "entryTime") {
          prompt = `당신은 고등학교 교무부 담당 교사입니다.
수능 또는 모의평가 입실 시간을 안내해주세요.

[입력 정보]
${contextDescription || "시험 유형 정보가 아직 입력되지 않았습니다."}

[중요] 참고 문서가 제공된 경우, 해당 학교의 입실 시간 안내를 참고하세요.

[작성 지침]
- 1교시 입실 마감 시간 (보통 08:10~08:20)
- 2교시 이후 입실 기준 (시험 시작 전 몇 분)
- 학교별로 입실 시간이 다를 수 있음

[출력 형식]
JSON 객체로 출력. 코드블록 없이 순수 JSON만:
{"entryTimeFirst":"1교시 입실시간","entryTimeOthers":"2~5교시 입실기준"}`;
        } else if (fieldName === "additionalNotes") {
          prompt = `당신은 고등학교 교무부 담당 교사입니다.
수능 또는 모의평가 관련 추가 안내사항을 작성해주세요.

[입력 정보]
${contextDescription || "시험 유형 정보가 아직 입력되지 않았습니다."}

[중요] 참고 문서가 제공된 경우, 해당 학교의 안내사항을 참고하여 다양하게 작성하세요.

[작성 지침]
다음 중에서 학교 상황에 맞는 내용을 선택하여 3~5개 작성:
- 점심 식사 안내 (도시락/급식 운영 여부)
- 복장 및 소지품 안내
- 등하교 시간 및 교통편 안내
- 시험 종료 후 일정 안내
- 성적 발표 일정
