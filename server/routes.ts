...
`;
        ...
        // 랜덤 스타일 선택을 위한 옵션들
        const greetingStyles = ["격식체", "친근한 격식체", "간결체", "서정적 표현"];
...
[이번 생성 스타일: ${randomStyle}]
[시작 방식: ${randomStart}]

[규칙]
1. 참고 문서에서 마음에 드는 표현/어휘를 골라 재조합
2. 위에 지정된 스타일과 시작 방식을 반드시 따를 것
3. 3~6문장으로 작성
4. 이전에 생성한 것과 다른 문장 구조 사용

인사말만 출력:`;
        } else if (fieldName === "cautions") {
          const orderOptions = ["전자기기부터", "신분증부터", "입실시간부터", "부정행위부터"];
          const randomOrder = orderOptions[Math.floor(Math.random() * orderOptions.length)];
          const formatOptions = ["번호(1. 2. 3.)", "글머리기호(•)", "대시(-)", "화살표(→)"];
          const randomFormat = formatOptions[Math.floor(Math.random() * formatOptions.length)];

          prompt = `[핵심 지시] 참고 문서의 유의사항을 분석하고, 다양한 표현을 조합하여 새로 작성하세요.
...
- 시작 항목: ${randomOrder}
- 형식: ${randomFormat}
- 항목 수: ${7 + Math.floor(Math.random() * 5)}개

[규칙]
1. 참고 문서들의 표현을 섞어서 새로운 조합 생성
2. 같은 내용도 다른 어휘로 표현 (예: "반입금지" vs "소지불가" vs "지참할 수 없음")
3. 항목 순서를 위 설정대로 배치
...
JSON 문자열 배열만 출력:`;
          ...
          const timeVariations = [
            {first: "08:10", others: "10분 전"},
            {first: "08:20", others: "시험 시작 10분 전"},
            {first: "08:10까지", others: "각 교시 시작 10분 전까지"},
          ];
          const randomTime = timeVariations[Math.floor(Math.random() * timeVariations.length)];

          prompt = `[핵심 지시] 참고 문서의 입실 시간 안내 표현을 참고하여 작성하세요.

[입력 정보]
${contextDescription || "시험 정보 없음"}

[참고 기준: 1교시 ${randomTime.first}, 이후 ${randomTime.others}]

[규칙]
1. 참고 문서의 시간 표현 방식 차용
2. "까지" vs "이전" vs "전까지" 등 표현 다양화

JSON 객체만 출력:
{"entryTimeFirst":"","entryTimeOthers":""}`;
        } else if (fieldName === "additionalNotes") {
          const topicPool = [
            "점심 도시락 지참", "급식 운영 안내", "복장 안내", "교통편 안내",
